...
const makeInput = (p: Props = {}) => {
  return <InputText labelText="label" placeholder="placeholder" type="text" disabled={false} required={true} readOnly={false} {...p} />;
};
...
describe('<InputText />', () => {
  describe('comportamento padrão', () => {
    ...
    test('aceita outras props do JSX (data-testid, maxLenght)', async () => {
  ...
  describe('acessibilidade', () => {
    test('não exibe mensagem de erro por padrão', async () => {
      ...
      expect(el).not.toHaveAttribute('aria-describedby', 'false');
      expect(screen.queryByRole('alert')).not.toBeInTheDocument();
    });

    test('não marca o input como válido por padrão', async () => {
      const el = input();
      expect(el).toHaveAttribute('aria-invalid', 'false');
    });
  ...
  describe('comportamento interativo', () => {
    test('aplica classes visuais quando desabilitado', async () => {
      ...
      expect(el).toHaveClass('disabled:text-slate-400');
    });
    ...
    test('mantém classes personalizadas do desenvolvedor', async () => {
      const el = input({ className: 'Vasco' });
      expect(el).toHaveClass('Vasco');
    });
  });
});