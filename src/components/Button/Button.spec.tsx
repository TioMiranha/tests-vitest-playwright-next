...
describe('<Button />', () => {
  describe('props padrão e JSX', () => {
    ...
    test('Verifies that default properties in JSX work correctly', async () => {
  ...
  describe('variants (cores)', () => {
    test('checa se default aplica a cor correta', async () => {
      render(<Button variant='default' title='O button'>Enviar formulário</Button>);

      const button = screen.getByTitle(/O button/i);

      expect(button).toHaveClass(VARIANT_DEFAULT_CLASSES);
    });

    test('checa se danger aplica a cor correta', async () => {
      render(<Button variant='danger' title='O button'>Enviar formulário</Button>);
    ...
    test('checa se ghost aplica a cor correta', async () => {
      render(<Button variant='ghost' title='O button'>Enviar formulário</Button>);
  ...
  describe('size (tamanhos)', () => {
    test('tamanho sm deve ser menor', async () => {
      render(<Button size='sm' data-testid="seilaLek">Enviar formulário</Button>);

      const button = screen.getByTestId(/seilaLek/i);

      expect(button).toHaveClass(SIZE_SM_CLASSES);
    });

    test('tamanho md deve ser médio', async () => {
      render(<Button size='md' data-testid="seilaLek">Enviar formulário</Button>);
    ...
    test('tamanho lg deve ser grande', async () => {
      const { container } = render(<Button size='lg' id="o-id">Enviar formulário</Button>);
