...
test.describe('<Home /> (E2E)', () => {
  ...
  test.describe('Renderização', async () => {
    test('deve ter o title html correto', async ({ page }) => {
      await page.goto('/');
      await expect(page).toHaveTitle('Testes com Vitest e Playwrigth');
    });
  ...
  test.describe('Criação', () => {
    ...
    test('deve fazer o trim da descrição do input ao criar TODO', async ({ page }) => {
      const { btn, input } = getAll(page);

      const textTobeTrimed = '  no spaces here   ';
      const textTrimed = textTobeTrimed.trim();

      await input.fill(textTobeTrimed)
      await btn.click();

      const createdTodo = page.getByRole('listitem').filter({ hasText: textTrimed });
      const createdTodoText = await createdTodo.textContent();

      await expect(createdTodoText).toBe(textTrimed);
    });
  ...
  test.describe('Exclusão', () => {
    test('deve permitir apagar um todo', async ({ page }) => {
      ...
      const itemToDelete = page.getByRole('listitem').filter({ hasText: todos[1].description });
    ...
    test('deve permitir apagar todos os TODOs', async ({ page }) => {
      ...
      while (true) {
        ...
        const renewedItem = page.getByRole('listitem').filter({ hasText: text });
        await renewedItem.waitFor({ state: 'detached' });
        await expect(renewedItem).not.toBeVisible();
      }
    });

    test('deve desativar os itens da lista enquanto envia a action', async ({ page }) => {
      await insertTestTodos();
      await page.reload(); // make next.js revalidate cache
      ...
      const allDeleteButtons = await page.getByRole('button', { name: /^apagar:/i }).all();

      for (const btn of allDeleteButtons) {
        await expect(btn).toBeDisabled();
      }

      const deletedItemNotVisible = page.getByRole('listitem').filter({ hasText: itemToBeDeletedText });
      await deletedItemNotVisible.waitFor({ state: 'detached' });
      await expect(deletedItemNotVisible).not.toBeVisible();

      const renewedAllButtons = await page.getByRole('button', { name: /^apagar:/i }).all();
  ...
  test.describe('Erros', () => {
    ...
    test('deve mostrar erro se um TODO já existe com essa descrição', async ({ page }) => {
      const { input, btn } = getAll(page);
